...
function createNewBox(cont, lbl) {
  ...
  // Add new voice in menu and relative listener
...
function saveParameters() {
  ...
  .then(text => {
    openModalMessage('Salvataggio opzioni', '<br><b>config.json</b> file salvato con successo sulla flash memory!<br><br>');
  });
}
...
function doConnection() {
  ...
  .then(function(text) {
    if (httpCode === 200) {
      openModalMessage('Connessione al WiFi', '<br>' + text + '');
    }
    else {
      openModalMessage('Error!', '<br>Errore di connessione: <b>' +  text + '</b><br><br>');
    }
    $('loader').classList.add('hide');
  });
}
...
function restartESP() {
  ...
  .then(data => {
    closeModalMessage();
    openModalMessage('Restart!', '<br>ESP restarted. Attendi qualche secondo e ricarica questa pagina.<br>');
  });
}
